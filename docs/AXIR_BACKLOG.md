...
- `axir-2026-08-25-port-shared-runtime-capability-selection-and-conformance-observa` [axagent] Port shared runtime capability selection and conformance observations
  ...
  - Impact: TypeScript AxCodeRuntime now exposes an explicitly versioned superset and conversion path for the generated AxIR inspect/snapshot/patch/abort/language/usage-instructions record, plus protocol/platform/persistence/resource/authority declarations, host-admitted security selection, and deterministic contradiction reporting; generated Python, Java, C++, Go, and Rust RuntimeCapabilities records and selectors do not yet expose the v1 extension or matching behavior.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
...
- `axir-2026-08-26-port-gepa-built-in-maxexamples-0-optional-reflectiveexamples` [axoptimize] Port GEPA built-in maxExamples 0 optional reflectiveExamples
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-08-26-port-playbook-historical-retention-policy-and-receipts` [axagent] Port playbook historical retention policy and receipts
  - Status: open
  - Source PR: #14
...
- `axir-2026-08-31-preserve-native-file-content-through-provider-routing` [axai] Preserve native file content through provider routing
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-a-call-time-skill-binding-is-admitted-through-both-catalog-gates` [axagent] A call-time skill binding is admitted through both catalog gates, or the run is refused
  - Status: open
  - Source PR: #98
  - Source commit: `b3c28f0339fdafaa0069a24b6261f72f9ded4a7e`
  - TS paths: `src/ax/agent/callTimeSkills.ts`
  - Impact: Review-driven refinement of the call-time skill injection entry for PR #98. New portable rule: a callTimeSkills binding that names a skillsCatalog ID is re-resolved at RUN START through the same two catalog gates every other retrieval path uses - requires-eligibility against the run's declared skill environment, and the retrieval-time authority re-check that produces the denied set - and the run is REFUSED with a distinct ineligible_bound_skill / denied_bound_skill detail when either gate hid the skill. This is fail-closed by design: silently dropping the skill while still intercepting would leave the actor blocked on a call it may not make with no procedure to read, and silently executing would make both gates advisory. The ordering is part of the rule: the binding table must be opened AFTER the retrieval gate is computed, because the gate is time- and authority-varying (an expired grant or revoked trajectory parks a skill mid-lifecycle) while a binding is static host configuration. An INLINE skill is not gated - host-supplied literal text has nothing to be eligible for and no provenance to re-check, the same posture as preset skills. Two further portable rules land with it: a host 'when' predicate (or the committed-state read behind it) that THROWS falls through to the normal call path rather than propagating, because the interception runs synchronously inside the actor's await and a propagated throw would become an error turn AND would never charge the injection budget, leaving the re-draft loop unbounded; and a predicate result that is not exactly true does not intercept. maxInjections is bounded above at 100.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-ace-optimizer-only-bullet-visibility-rejected-retained-verificat` [axgen] ACE optimizer-only bullet visibility, rejected-retained verification, model-facing provenance redaction, and retrieval-time authority re-check
  - Status: open
  - Source PR: #97
...
- `axir-2026-09-02-agent-playbook-evolve-matched-budget-control-arm-neutral-artifac` [axagent] Agent playbook evolve: matched-budget control arm, neutral-artifact harness-term ablation, and the baseline/evolved snapshot state machine
  - Status: open
  - Source PR: #93
  - Source commit: `0b0e0110e746ff17ed3fb7c841d3b5a3e2bdd485`
  - TS paths: `src/ax/agent/agentInternal/playbookEvolve/controlArm.ts`, `src/ax/agent/agentInternal/playbookEvolve/snapshots.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvolve.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvidenceTypes.ts`
  - Impact: Adds a default-off matched-budget control arm to agent.playbook().evolve(). Portable TS behavior changes: (1) runAgentEvalBatch gains an optional metricTaskOf seam that maps the task handed to the agent onto the task handed to the metric (identity unless supplied, so the legacy path is unchanged); (2) evolve() gains run-level phases 0/7/9/10: it captures the unevolved and evolved artifacts unconditionally, restores the unevolved one under canonical digest assertions to run best-of-N, self-refinement and a neutral-artifact ablation from a SEPARATE metric-call counter whose spend still lands in accounting.metricCalls but never in the legacy metricCallsUsed, and can then roll the WHOLE accepted set back when gates.controlArm is 'require'; (3) a run-level rollback cascades applied='rolled_back', playbookSnapshot=undefined, rolledBackReason, evidence.decision='superseded' with a re-digested receipt, and promotion.status='promoted_then_rolled_back'. Type-level widening (non-breaking): AxAgentPlaybookEvidenceWarningCode gains control_arm_not_beaten and control_arm_unmeasured; AxAgentPlaybookControlArmResult gains optional neutralArtifactDigest/neutralArtifactTokens; AxAgentPlaybookEvolveError gains an optional playbookSnapshot; AxAgentPlaybookEvolveOptions gains controlArm. Generated packages keep the legacy gate; the control arm is TypeScript-first. Fail-closed semantics on the run-level gate: an arm whose every deciding-split task went unscored is reported as skipped rather than as a mean of 0, and the gate fails closed on an unmeasured comparison (no result for the named best arm, an incomplete best arm, or an interval with no clusters/resamples). AxAgentPlaybookControlArmResult.n reports what the arm ACTUALLY ran (samples drawn, rounds re-invoked), never the planned figure. A dry run (apply: false) whose required gate fails stays applied='dry_run' and runs no I8 cascade.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-agent-playbook-evolve-per-cell-transfer-matrix-with-a-regression` [axagent] Agent playbook evolve: per-cell transfer matrix with a regression floor, and a sealed test evaluated once after the run-level verdict
  - Status: open
  - Source PR: #104
  - Source commit: `1c6d8a0fe7c2914bb609a717fbaadc0aa3915671`
  - TS paths: `src/ax/agent/agentInternal/playbookEvolve/transfer.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvolve.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvolveTypes.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvidenceTypes.ts`
  - Impact: Adds two default-off evidence options to agent.playbook().evolve(). Portable TS behavior changes: (1) transfer evaluates every caller-supplied AxAIService target on every configured split twice (an anchor pass before any mutation and a candidate pass on the final artifact) and reports ONE CELL PER (target, split) with its own paired interval and a per-cell regression floor (default 0.02); the report carries no mean/average field by contract, and gates.transfer: 'require' triggers the existing run-level rollback, whose I8 cascade now names the deciding gate instead of a hard-coded control_arm. The transfer verdict fails closed on a missing planned cell, an incomplete pass, or an unpairable anchor/candidate pair, and a target that throws makes the whole matrix not_run. (2) sealedTest is proven disjoint from train and validation by semantic task id and is evaluated EXACTLY ONCE in phase 11, after the run-level verdict and after the rollback, on the baseline artifact and then the final artifact inside digest-verified restore brackets; no gate, threshold, accept decision or rollback reads it, and a run whose final artifact digest equals the baseline's reports not_run rather than a delta that measures run-to-run noise. Both spend from their own metric counters, so neither moves the legacy metricCallsUsed and neither can starve the search; an insufficient transfer.maxMetricCalls raises budget_insufficient before any mutation. AxAgentPlaybookEvidenceWarningCode gains transfer_cell_regressed and transfer_unmeasured. Generated packages keep the legacy gate; the new options are TypeScript-first. Review-driven refinements in the same PR: the sealed test also reports not_run when either pass could not finish the whole split (budget exhausted, or every attempt at a task discarded), because a prefix mean (and a pass that scored nothing, whose mean defaults to 0) is not a test number; its budget is therefore sized 2 x |sealed| x runsPerTask x (1 + maxDiscardRedraws). Every (target, split) transfer pass draws from its own sub-budget of |split| x runsPerTask x (1 + maxDiscardRedraws) so one target's re-draws cannot truncate another's pass, and a pass that exhausted its budget is named as exhausted rather than read as a regression. A non-'off' transfer gate that fails without rolling anything back (a dry run, or zero accepts) reports the failure on its warning, so it is never indistinguishable from gates.transfer: 'off'. A restore failure around phase 11 raises the new AxAgentPlaybookEvolveErrorCode member 'sealed_test_failed' in phase 'sealed_test' rather than control_arm_failed.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-agent-playbook-evolve-termination-classification-per-attempt-evi` [axagent] Agent playbook evolve: termination classification, per-attempt evidence, bootstrap intervals, variance band, validity conjuncts, reach instrumentation, gate chain and evidence receipt
  - Status: open
  - Source PR: #88
  - Source commit: `51dab998e83985e29443d65e91a4f928564ac068`
  - TS paths: `src/ax/agent/agentInternal/playbookEvolve/playbookEvolve.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvolveTypes.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvidenceTypes.ts`, `src/ax/agent/agentInternal/playbookEvolve/evalHarness.ts`, `src/ax/agent/agentInternal/playbookEvolve/gates.ts`, `src/ax/agent/agentInternal/playbookEvolve/statistics.ts`, `src/ax/agent/agentInternal/playbookEvolve/termination.ts`, `src/ax/agent/agentInternal/playbookEvolve/validity.ts`, `src/ax/agent/agentInternal/playbookEvolve/reach.ts`, `src/ax/agent/agentInternal/playbookEvolve/accounting.ts`, `src/ax/agent/agentInternal/playbookEvolve/evidenceReceipt.ts`, `src/ax/agent/agentInternal/playbookEvolve/canonical.ts`, `src/ax/index.ts`
  - Impact: BREAKING (type-level): AxAgentPlaybookEvolveResult gains required control/accounting/applied and AxAgentPlaybookEvolveOutcome gains required kind, so construction sites (host test doubles, adapters) must be updated; applied is a three-state discriminant ('live'|'dry_run'|'rolled_back'), not a boolean. AxAgentEvalBatchResult, exported from the package root, likewise gains four REQUIRED properties (discardedRuns, durationMs, usage, termination), so a host-built batch result no longer compiles unchanged. AxAgentPlaybookEvolveProgressEvent.phase widens from 5 to 12 members, which breaks an exhaustive switch. Portable TS behavior changes: evaluation-batch completeness now accounts for discarded environment-failure attempts (executedRuns + discardedRuns === expectedRuns) AND a task whose attempts are all discarded no longer aborts the batch (continue, not break), with truncatedAtTaskIndex set only for genuine budget exhaustion; the accept decision gains optional validity/interval/reach conjuncts, all default-off; a bounded re-draw may re-run a discarded attempt from the same metric budget; the gate chain evaluates its two host-call gates lazily so a candidate an earlier required gate already rejected never pays for one. Token accounting distinguishes 'unreported' (the phase could report usage and none arrived) from 'unobservable' (the phase cannot report without a usageTap). Generated packages keep the legacy gate; the new options are TypeScript-first and the gate chain is not yet expressible in Core IR. Conformance fixture ir/conformance/axagent/playbook-evidence-gate.json pins the gate order, the host-call short-circuit, the prune loss-tolerance variant of gate 1, and completeness-under-discards; its assertions run in scripts/playbook-evidence-conformance.test.ts until the behavior is migrated.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-agent-promotion-gate-extracted-to-playbookevolve-gate-ts-for-reu` [axagent] Agent promotion gate extracted to playbookEvolve/gate.ts for reuse by the learning surface
  - Status: open
  - Source PR: #86
...
- `axir-2026-09-02-axmind-monolith-and-responder-thinkers-the-model-free-determinis` [runtime] AxMind monolith and responder thinkers, the model-free deterministic program, and the host-mediated function menu
  - Status: open
  - Source PR: #101
  - Source commit: `48c4e8446f9bb6810e007fbd208b396d1566d6bc`
  - TS paths: `src/ax/mind/thinkers.ts`
  - Impact: New portable TS behaviour: a thinker rendered as an event target (ai plus exactly one of program/createProgram plus a context assembler in mapInput position, so a broken projection dead-letters before any model call); the monolith's function menu (act/think/share/learn/goals/idle) with every handler routed through one boundary the runtime decides close_from_inside on, and a goal change recorded as a proposal because an artifact write needs an out-of-band host receipt; the responder as a single generation with a chat-shaped context and an inner-life block whose reply and whose decline both go through the ledgered chat path; goal rendering with AxACEBulletLifecycle vocabulary (status, expiresAt, supersededBy, reason) and priority ordering; and a deterministic program that satisfies the whole programmable surface over one run so a model-free thinker still composes. Not yet expressed in AxIR.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-axmind-pacing-ladder-wake-routing-with-source-suppression-cursor` [runtime] AxMind pacing ladder, wake routing with source suppression, cursor-deferred trajectory source, lag health, ledgered chat idempotency, salience injection, and the kernel/catalog skill tier
  ...
  - TS paths: `src/ax/mind/types.ts`, `src/ax/mind/pacer.ts`, `src/ax/mind/routes.ts`, `src/ax/mind/sources.ts`, `src/ax/mind/health.ts`, `src/ax/mind/chat.ts`, `src/ax/mind/salience.ts`, `src/ax/mind/skills.ts`
  - Impact: Portable-in-spirit state machines a non-TypeScript target would need to reproduce: the backoff ladder (delay(0)=0, delay(n)=min(base*factor^(n-1), cap), dwell, thought ceiling, immediate error descent, unchanged-on-noop, and the absolute wakes-per-hour fuse) plus its reconstruction from the log alone; the dispatch decision table (registry-derived wakeability, source- and launchedBy-based self-suppression, unconditional own-error suppression, admission-bound deferral that never drops, wake-signal coalescing with a visible count); lag-based health thresholds; the rate fuse derived as the steady-state tolerance PLUS the ladder descent cost, with a published drain time and an edge-triggered pace duty so a parked or unchanged wake cannot re-fire per tick; the disjunctive chat idempotency key and the priority-ordered reply-state resolution, in which outbound authorship is the host-stamped writer identity rather than the remote-controlled from field, a claim TTL fails open, and another thinker's decline cannot cancel the claim owner's reply; and the kernel/catalog skill selection with whole-entry demotion and host-facts-only requires gating.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-axmind-review-fixes-per-step-token-delta-settle-after-sinks-and-` [runtime] AxMind review fixes: per-step token delta, settle-after-sinks, and the liveness-fallback arm
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-axmind-sibling-wake-suppression-per-effect-chat-send-attribution` [runtime] AxMind sibling wake suppression, per-effect chat send attribution, and a read-only trajectory reader
  - Status: open
  - Source PR: #103
  - Source commit: `9f84a0befb373f0d2f35e5614b6202640218f717`
  - TS paths: `src/ax/mind/routes.ts`
  - Impact: Dispatch: a step whose writer is a SIBLING thinker of the same mind and whose type is in the derived sibling-inert class (registry wakeSignal, a wakeable type carrying no content, or neverRetriggersSelf) does not create a delivery; the refusal is reported as the wake-suppressed-sibling diagnostic. Crash C10 reconcile attributes a rebuilt message step to the thinker recorded on the settled effect, failing closed to the caller identity when that name is not in the mind's own thinker table. AxTrajectoryReader narrows the trajectory handle a thinker receives to the read primitives.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-axmind-step-orchestration-wake-classification-work-probe-bracket` [runtime] AxMind step orchestration: wake classification, work-probe-bracketed runs, budget dimensions, and fork/run/merge sub-runs
  - Status: open
  - Source PR: #101
  - Source commit: `48c4e8446f9bb6810e007fbd208b396d1566d6bc`
  - TS paths: `src/ax/mind/mind.ts`
  - Impact: New portable TS behaviour a non-TypeScript target would need to reproduce: wake classification from the published envelope (reactive / spontaneous / watchdog / bootstrap / manual) with a synthesized trigger for the wakes that have no step behind them; one thinker step bracketed by two bounded filtered work probes so the outcome is a log fact rather than a parse of model output; the arming finally that arms the next spontaneous wake on every exit path including a throw inside the orchestration itself; the four budget dimensions (wall clock, tokens, sub-runs, depth) with the typed AxMindBudgetExceededError; the deterministic routing-signal table computed only from the bounded projection the model is about to read; and fork -> run -> merge sub-runs that always merge something back, success, failure or cancellation. Not yet expressed in AxIR.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-axtrajectory-append-only-step-log-blob-spill-filtered-backward-t` [runtime] AxTrajectory append-only step log, blob spill, filtered backward tail, and durable per-consumer cursors
  ...
  - TS paths: `src/ax/trajectory/types.ts`, `src/ax/trajectory/registry.ts`, `src/ax/trajectory/spill.ts`, `src/ax/trajectory/log.ts`, `src/ax/trajectory/memoryStore.ts`, `src/ax/trajectory/conformance.ts`
  - Impact: New portable TS behaviour: append-only step model with dense gap-free per-trajectory seq, registry-declared narrative/machinery/structural classification with an open-world unknown descriptor, generic size-based blob spill with a digest-verified read-side resolver and a single-fetch batch pre-pass, bounded reads throughout (no unbounded read, maxScan-bounded filtered backward tail reporting scanned beside exhausted, step and byte drain budgets, a 256-id fetch cap), durable per-consumer cursors, and a fork/merge DAG with bidirectional references. Steps are deep-frozen once indexed, so append-only immutability is a runtime property rather than a type-level one, and a durable cursor resolves its frame location against the index so a tolerant parse that drops an interior frame fails closed instead of silently renumbering every later step. Pinned by the executable runAxTrajectoryStoreConformance contract. Not yet expressed in AxIR.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-axtrajectory-tiered-rollup-context-projection-budget-rule-stairc` [runtime] AxTrajectory tiered-rollup context projection: budget rule, staircase assembly with straddle descent, forward-only enablement, and drill-down by cited step id
  - Status: open
  - Source PR: #91
  - Source commit: `95ad4abec618e176b48fa02c7de375c958ac24bd`
  - TS paths: `src/ax/trajectory/projection.ts`, `src/ax/trajectory/rollups.ts`
  - Impact: New portable TS behaviour: a context budget rule of min(fraction * host-supplied window, absolute cap) with a derived raw-tail size floored at 20; a narrative-only recent stream filtered through the registry's declared stepClass so machinery and unknown steps cannot enter a projection even when named explicitly; F-ary tiered rollup blocks keyed by absolute filtered-step index range, sealed immutable and stamped with (summarizerId, promptVersion); a staircase assembler that decomposes the pre-tail prefix positionally in base F and emits it in chronological order, prunes segments below a forward-only enablement marker snapped down to an F boundary before descending, descends into the F children of a missing coarse block that straddles the marker, and reports only a tier-1 miss as a visible gap; and drill-down by cited step id where a citation outside its own block's range is dropped at seal time. The bounds are part of the behaviour: maxBlocks bounds summarizer ATTEMPTS per build rather than successes, the staircase descent is bounded by F^2 * (ceil(log_F cut0) + 1) assembler nodes with the remainder reported as a missing gap, summaries and themes are clipped at seal time, every interpolated value in the render is one-lined so model output and step bodies cannot forge a section header or a verbatim frame, and a rollup checkpoint sealed past the end of the log it is loaded for is rejected as a meta conflict. Not yet expressed in AxIR.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-call-time-skill-injection-with-a-speculation-adapter-interlock` [axagent] Call-time skill injection with a speculation-adapter interlock
  - Status: open
  - Source PR: #98
  - Source commit: `e0167695dc78690204b890be0c192f4c38842208`
  - TS paths: `src/ax/agent/callTimeSkills.ts`
  - Impact: Adds an opt-in per-callable binding to the actor loop: when the actor drafts a call to a bound qualified callable the harness returns a frozen not-executed marker BEFORE the authority boundary instead of dispatching, so the call requests no authorization, fires no onFunctionCall, produces no function-call record and mints no working-state tool receipt; the bound skill is then ingested through the existing loaded-skills channel and harness-authored guidance is appended to the trusted guidance log so the model re-drafts on the next turn. A bound callable is additionally excluded from the JS-runtime speculation adapter, so the speculation path cannot execute, authorize or mint behind the interception. Injections are budgeted per callable (default 1), an optional predicate over the committed working state gates them, and the trace records the intercepted turn as not executed. Default behaviour is unchanged when callTimeSkills is unset.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-core-ir-event-route-matching-covers-types-and-sources-only-no-su` [runtime] Core IR event route matching covers types and sources only: no subject matching, no extension matching, no authorize predicate
  - Status: open
  - Source PR: #101
  - Source commit: `48c4e8446f9bb6810e007fbd208b396d1566d6bc`
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-deferred-ast-level-mutation-annotation-for-program-source-candid` [axprogram] Deferred: AST-level mutation annotation for program-source candidates
  - Status: open
  - Source PR: #100
  - Source commit: `382c3542be9ae312e69f9e5876ece7d1f5f5f5d6`
  - TS paths: `src/ax/dsp/programSource.ts`
  - Impact: AxMutationAnnotator is invoked once per GEPA candidate, not once per program-source AST edit. programSource.ts has no annotation hook, so a candidate that rewrites an evolved AST is classified as a single program.source_replace patch regardless of how many distinct edits it made. Recorded so the gap is visible rather than implied by the taxonomy's presence.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-gepa-evidence-primitives-branded-digests-trajectory-admission-mu` [axoptimize] GEPA evidence primitives: branded digests, trajectory admission, mutation taxonomy, harness recipes, discriminative sampling with IPW, candidate effect declarations, rejected-candidate ledger
  - Status: open
  - Source PR: #89
  - Source commit: `50f3de89c2dd03e38c9b66324b749dd4cab87d91`
  - TS paths: `src/ax/dsp/optimizers/digests.ts`, `src/ax/dsp/optimizers/trajectoryTermination.ts`, `src/ax/dsp/optimizers/mutationTaxonomy.ts`, `src/ax/dsp/optimizers/harnessRecipe.ts`, `src/ax/dsp/optimizers/taskDiscrimination.ts`, `src/ax/dsp/optimizers/candidateEffectManifest.ts`, `src/ax/dsp/optimizers/rejectedCandidateLedger.ts`, `src/ax/dsp/optimizers/gepaLineage.ts`
  - Impact: Adds seven pure, currently UNWIRED modules under src/ax/dsp/optimizers/ and narrows fingerprintGEPAValue's return type. Nothing here is reachable from AxGEPA, so portable optimizer behaviour is byte-identical (proved by test:gepa-upstream-compatibility against the merge base). The portable semantics these modules will carry once wired: three digest strengths with a runtime strength assertion; host-classified trajectory termination with a non-reclassifiable override for config errors and program-source failures, admission accounting, a run-level discard ceiling, and an intersected paired-admitted index set; a mutation depth/patch taxonomy keyed on component kind that fails closed on an unmapped kind; harness recipes digesting bindings plus a bound model id; Beta(1,1)-smoothed Bernoulli-variance inclusion probabilities with a mandatory exploration floor, water-fill, Madow systematic pps sampling consuming exactly one RNG draw, and a Hajek/IPW paired-difference estimator; structural validation of candidate effect declarations bound to AxEventEffectIntent; and a rejected-candidate ledger port whose entries expire by construction with fail-open unknown context. Two ordering rules are load-bearing for any port: array members inside an identity digest (harness recipe bindings, ledger component deltas) are sorted by UTF-16 code unit, never by locale collation, so the digest is a function of its input alone; and the rejected-candidate prior block is a distinct type from the trusted optimization-reference channel and must never be renderable inside TRUSTED markers.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-gepa-lineage-version-2-mutation-taxonomy-reflection-outcomes-har` [axoptimize] GEPA lineage version 2: mutation taxonomy, reflection outcomes, harness stamp, and the deployable best chain
  - Status: open
  - Source PR: #100
  - Source commit: `382c3542be9ae312e69f9e5876ece7d1f5f5f5d6`
  - TS paths: `src/ax/dsp/optimizers/gepaLineage.ts`
  - Impact: AxGEPACandidateLineageManifest becomes version 1 | 2. Version 2 adds five optional record fields (mutation annotation, paired reflection outcomes, causal-evidence cross-link id, harness stamp, per-batch admission) and five optional manifest fields (mutation depth histogram, discrimination summary, harness stamp, deployable best chain, run-level admission). Two new compile options, harnessRecipe and mutationAnnotation, are read through the Proxy-safe own-data reader; a required annotation aborts a candidate before either promotion gate evaluates it. The manifest version is derived from content, so a record carrying a version-2 field can never ship inside a version-1 manifest. With every opt-in option absent the manifest stays version 1 and byte-identical, including the rand() draw sequence. The lineage objectives key order moves from localeCompare to code-unit order, so it no longer depends on the host locale.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-gepa-rejected-candidate-ledger-wiring-and-the-untrusted-proposer` [axoptimize] GEPA rejected-candidate ledger wiring and the untrusted proposer prior
  - Status: open
  - Source PR: #100
  - Source commit: `382c3542be9ae312e69f9e5876ece7d1f5f5f5d6`
  - TS paths: `src/ax/dsp/optimizers/gepa.ts`
  - Impact: Rejected candidates are recorded into a host-owned store at five sites (both merge aborts, the merge reject, the mutation abort and the mutation reject) with the gate reading, the estimator the gate actually used, the mutation annotation, the harness stamp and a bounded untrusted diagnosis. Each round reads the prior for the components it is about to propose, with expiry evaluated at query time against the caller's context and an injected clock; an expiresWhen with no after_ms clause is refused before the first metric call. AxGEPAProposalPolicyArgs gains rejectedPrior, rendered into its own delimited untrusted prompt field and never into the trusted optimization-reference channel; GEPA_PROPOSAL_CONTRACT gains two sentences only when a non-empty prior is present and is byte-identical otherwise. AxGEPAOptimizationReference gains an optional channel discriminant that makes the untrusted block structurally unassignable to it. AxOptimizedProgram gains rejectedCandidateLedgerRef, a pointer set the artifact cannot rewrite. A store that throws degrades to a lineage runtime failure and never aborts the run; a failed read degrades the same way, and an abort is reported as an abort rather than as a store fault. A gate reading carries parentScore and childScore together or not at all: an abort for insufficient_admitted_rows records neither and no observed delta, and an ipw_hajek reading carries a differenceEstimate instead of a score pair. The artifact's pointer set is bounded to 256 digests OLDEST-FIRST at every path that builds or unions one, and a deserialized ref is normalized through the same validator as a written one.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-gepa-trajectory-admission-at-both-promotion-gates-and-opt-in-dis` [axoptimize] GEPA trajectory admission at both promotion gates and opt-in discriminative minibatch selection
  - Status: open
  - Source PR: #92
  - Source commit: `02685baabdd047c06f918a0f45cd5eed47bda0f7`
  - TS paths: `src/ax/dsp/optimizers/gepa.ts`
  - Impact: Wires host-owned trajectory termination classification into evaluateGEPABatch so environment-failed rows leave the promotion denominator with the discard rate reported, intersects the admitted index sets at BOTH of GEPA's promotion gates (reflective mutation and the default-on system merge, via a new perInstanceAdmitted mask), and adds an opt-in discriminative minibatch strategy with Beta(1,1)-smoothed water-filled inclusion probabilities, Madow systematic pips sampling and a Hajek/IPW paired-difference accept branch. A port must reproduce: the all-rows meaning of avg/scalars/sum under admission; the non-reclassifiable override for config-error rows and for any row of a program-source candidate; the inconclusive-batch abort and the run-level discard ceiling that publishes no best score; the intersected denominator at both gates; and the fact that with every option omitted the artifact, events, checkpoints, selection and rand() draw sequence are unchanged.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-kernel-indexed-skill-tiers-with-requires-gating-per-skill-cost-a` [axagent] Kernel/indexed skill tiers with requires gating, per-skill cost accounting, verification budget, and bounded verifier rails
  - Status: open
  - Source PR: #97
  - Source commit: `3a9f187d01ce593115d2428ba666a98dd03abccb`
...
- `axir-2026-09-02-learning-surface-part-2-harness-tree-admission-gate-installer-no` [axoptimize] Learning surface part 2: harness tree, admission gate, installer, nominate/promote release chain, failure manifest, axHarnessEvolve
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-learning-surface-review-fixes-candidate-only-failure-manifest-ta` [axoptimize] Learning surface review fixes: candidate-only failure manifest, target-driven install, judge resolution, cycle classification
  - Status: open
  - Source PR: #96
  - Source commit: `38d5a2350e5a1dce12b126b32ba25068d903888d`
  - TS paths: `src/ax/learn/types.ts`
  - Impact: Corrects portable TS semantics shipped by the learning surface. (1) The failure manifest is advanced from the CANDIDATE side's observations only; observations carry the side they were seen on and the current side travels on the evaluation instead, so a nominated candidate never ships a durable claim that it failed the baseline's tasks. (2) Installing a tree is a function of the TARGET: whenever the target has a playbook handle the install replaces the playbook and the continuous-learning reset guard fires, including for a tree carrying no bullets; the skills slot is replaced whenever the tree fills it or the slot already holds something. (3) The built-in judge resolves judgeAI then the target's own getJudgeAI(), and refuses a teacherAI with neither, so proposer and evaluator are never correlated by default. (4) A tree that cannot be canonicalized is denied 'non-json-config' rather than raising RangeError, on every entry point. (5) The evolve step number is derived from the chain tail, matching publish. (6) publish holds the gate decision to the same persistability floor as a record payload. (7) A non-restorable rollback target raises the typed AxLearningRollbackRefusedError with a structural guard.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-playbook-evolve-gated-pruning-proposals-and-host-promotion-autho` [axagent] playbook evolve: gated pruning proposals and host promotion authority
  - Status: open
  - Source PR: #99
  - Source commit: `47ba0f0f104419697a7c37de31b117d08575e57f`
  - TS paths: `src/ax/agent/agentInternal/playbookEvolve/evidenceConformance.ts`, `src/ax/agent/agentInternal/playbookEvolve/playbookEvolve.ts`, `src/ax/agent/agentInternal/playbookEvolve/promotion.ts`, `src/ax/agent/agentInternal/playbookEvolve/proposals.ts`, `src/ax/agent/agentInternal/playbookEvolve/pruning.ts`, `src/ax/dsp/optimizers/acePlaybook.ts`
  - Impact: evolve() gains a prune phase whose REMOVE/DEPRECATE proposals run the full per-candidate gate chain in its loss-tolerance variant (gates 1/2 become maxCurrentLoss/maxHeldOutLoss, gate 7 prune_size requires a real rendered-token reduction), applied by a new snapshot-transform mutation primitive that replaces applyCuratorOperations' id/structural validation, stats recompute and artifact-history append explicitly so the derived bulletIds and the accept-path evidence stamp stay non-empty; a leave-one-out held-out redundancy sweep that never proposes a load_bearing or unresolved bullet; a rendered-size ceiling that triggers proposals and never truncates; curate-path section-overflow eviction instrumentation (outcome.evictions plus curate_eviction); and two host-callback gates where an AxAuthorizationReceipt bound to a caller-supplied host-grantable resourceId grants the capability while a conjunctive fail-closed promotionVeto decides the candidate, with promotion records distinguishing not_required/not_nominated/vetoed/denied/promoted. A port must reproduce the prune-variant thresholds, the never-truncate rule, the history-append-derived bulletIds, the fail-closed veto table (only false and { vetoed: false } decline) and the exact-resource-identity grant binding.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-register-the-axmind-conformance-suite-with-the-axir-verifier-and` [runtime] Register the axmind conformance suite with the AxIR verifier and the five target runners
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-skill-authority-provenance-extraction-and-retrieval-time-re-chec` [runtime] Skill authority provenance extraction and retrieval-time re-check at the host-authority boundary
  - Status: open
  - Source PR: #97
  - Source commit: `3a9f187d01ce593115d2428ba666a98dd03abccb`
...
- `axir-2026-09-02-skillstate-actor-memory-mode-frozen-skill-spec-plus-typed-state-` [axagent] skillState actor memory mode: frozen skill spec plus typed state as the actor prompt substrate
  ...
  - Impact: Adds an opt-in second prompt substrate to the actor loop. Under actorMemoryMode 'skillState' the prompt is a frozen skill spec, the declared state contract, the typed working-state document, the read-only receipt roster and the latest observation; the action log is neither replayed nor rendered nor checkpoint-summarized. The actor emits an optional typed RFC-6902 statePatch and a rationale that is digested and discarded; AxSkillStateRuntime validates the patch through the working-state kernel and host checker and commits it through AxProgramStateStore.compareAndSet under the configured fence, recording one transition per ACCEPTED transition with a non-optional committedRevision and a schema/authority/fence/invariant rejection vocabulary. The actor prompt measurement now reads the same value record the turn sends. Default behaviour is unchanged when actorMemoryMode is unset. Review-driven refinements ports must match: rationaleDigest is ABSENT when the actor emitted no rationale at all (an empty rationale still digests), so declining to explain is distinguishable from explaining with nothing; the rejection is decided by the working-state error CODE rather than by constructor identity; the kernel advances its stored envelope on the bounded rebase, so the envelope revision a host reads always equals the kernel current revision; and a code-policy-refused turn is recorded as the latest observation, which in this mode is the only history the actor sees.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
...
- `axir-2026-09-02-verifier-gated-typed-working-state-for-the-actor-loop` [axagent] Verifier-gated typed working state for the actor loop
  ...
  - TS paths: `src/ax/agent/statePatch.ts`, `src/ax/agent/workingState.ts`
  ...
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
- `axir-2026-09-02-version-4-causal-candidate-evidence-manifests-with-receipt-cover` [axoptimize] Version-4 causal candidate evidence manifests with receipt-covered policy
  - Status: open
  - Source PR: #100
  - Source commit: `382c3542be9ae312e69f9e5876ece7d1f5f5f5d6`
  - TS paths: `src/ax/dsp/optimizers/causalCandidateEvidence.ts`
  - Impact: AxCausalCandidateEvidenceManifest becomes version 3 | 4, bumped by ANY version-4 feature rather than by a declared policy alone, so a version-3 manifest keeps identifying the exact legacy record schema. Records gain attribution, mutation, cost, harness, discrimination (with an estimator and gate discriminant), admission, effects and runtimeRequirements; ablation gains a leave-one-out matrix held to the same link invariants as the single ablation it generalizes. validateManifestRecords takes an explicit policy; canonicalizeReceipt appends a version-guarded policy key so the manifest-level gate is covered by the authority receipt; axCloneCausalCandidateEvidenceManifest accepts requirePolicyAtLeast and re-validates under the strictest of the artifact's policy and the caller's. Every version-4 effect gate keys on the record's own affectedComponents (componentKind plus declared tool:* capabilities), never on the optional host-authored mutation annotation: a promoted record touching a component that declares a tool capability is refused without an effect declaration, a promoted record touching a program-source component is refused without runtime capability requirements, and an effect declaration on a steering patch or on a record naming no program-source surface is refused. Keying any of them on the annotation made both the gate and a reader's requirePolicyAtLeast floor defeatable by omitting one optional field. axReplaceOptimizedProgramSnapshot unions only the rejected-candidate ledger ref and leaves the divergent-causal-history refusal untouched. No version-4 manifest is readable by an older build.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
