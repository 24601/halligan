...
- `axir-2026-08-25-port-experimental-program-source-optimization` [axoptimize] Port experimental program-source optimization
  ...
  - Impact: TypeScript exposes programSource(), a validated ax-program-source/v1 control-flow AST with explicit predictor/tool capabilities, default locked-down worker interpretation and Node resource ceilings, strict typed outputs, bounded JSON bridge values, revocable execution epochs, explicit custom-runtime language/protocol compatibility, per-example call/iteration budgets, state serialization, and GEPA-aligned bind/runtime failures. Generated Python, Java, C++, Go, and Rust packages do not expose this program-source component, interpreted grammar, or runtime bridge contract.
