...
- Inputs, predictor/tool arguments and results, and final outputs must be plain
  acyclic JSON graphs within configurable UTF-8 byte, depth, and per-container
  width limits. Defaults are 1,048,576 bytes, depth 24, and width 4,096. Values
  fail closed before each next bridge/runtime use.
