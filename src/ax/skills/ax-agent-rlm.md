...
- `purity: 'pure'` is the application's attestation that starting and abandoning the call is safe: no writes, messages, transactions, completion-protocol calls, or other durable application effects. The speculative AxFunction adapter deliberately omits the completion protocol. The handler must honor `abortSignal` and tolerate cancellation.
...
- When an Ax host authority context is active, the exact operation/resource is authorized before any physical function work and the receipt is passed to that invocation. The receipt, grant state, expiry, identity, and lease epoch are revalidated at the real worker claim without calling host policy twice. A current denial is claimed with zero physical function work and, matching normal authorization order, no logical call observer or recorder; a revoked, expired, or otherwise invalidated future is discarded and the logical call takes the normal authorization/execution path. Hosts must update grants or the lease epoch when external policy changes so revocation is visible to Ax.
...
- Real worker execution claims by exact callable reference, canonical arguments, and nondeterministic occurrence. A hit adopts the already-started promise without retrying; a miss invokes the ordinary host function once. Its normal function-call observer and recorder commit exactly once at the real worker call. Unclaimed work is cancelled and never committed as a logical function call.
...
- `onEvent` receives best-effort `dispatch`, `hit`, `miss`, `blocked`, and `cancelled` diagnostics with bounded reason codes, including authority/launch invalidation misses. Callback errors are ignored; diagnostics are not an authorization mechanism.
