...
- `onEvent` receives best-effort `dispatch`, `hit`, `miss`, `blocked`, and `cancelled` diagnostics with bounded reason codes. Callback errors are ignored; diagnostics are not an authorization mechanism.
